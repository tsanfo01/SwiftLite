...
let check defns =
  let built_in_decls = 
    ...
    in List.fold_left add_bind StringMap.empty
        [ ("print", String); ("printi", Int); ("printb", Bool); ("printc", Char) ]
  in
  ...
  | ArrayLit (t, l) -> 
      ...
      let (ty, b) = List.fold_left (fun (t', b) t'' -> (t', t' = t'' && b)) (t, true) ts in
      if b
      then (Array ty, SArrayLit (t, slits))
      else raise (Failure "Polymorphic array not allowed")
  ...
  let expr scope funcs enums classes e =
    let rec ex = function 
      Literal l -> 
        let (t, sl) = literal l in
        (t, SLiteral sl)
    | Id(s) ->
        let (t, _) = try type_of_identifier s scope
                     with Not_found -> raise (Failure ("undeclared identifier " ^ s)) in
        (t, SId(s))
    | Assign(s, e1) ->
        let (t, e') = ex e1 in
        let (t', b) = try type_of_identifier s scope
                      with Not_found -> raise (Failure ("undeclared identifier " ^ s)) in
        if t = t'
        then if b
             then raise (Failure ("Variable " ^ s ^ " is constant in local scope"))
    ...
    | Binop(e1, op, e2) ->
        ...
        let ty =
          ...
          | Eq | Neq  when same &&
              (match t1 with Int | Float | Bool | Char -> true
                        | UserDef(s) -> StringMap.mem s enums | _ -> false)   -> Bool
          | Lt | Leq | Gt | Geq
    ...
    | Call(s, es) as call ->
      ...
      then
        ...
        let eqArgTys (args, _) =
          ...
          tys = tys'
        in
        let i = try List.find eqArgTys inits
                with Not_found -> raise (Failure ("no init matching given argument types")) in
        let si = s ^ (string_of_int (index_of i inits)) in
        (UserDef s, SInit (s, si, es'))
      else
        let fd = try find_func funcs s
                 with Not_found -> raise (Failure ("no such function " ^ s)) in
        let param_length = List.length fd.params in
        if List.length es != param_length
        then
          raise (Failure ("Expecting " ^ string_of_int param_length ^ 
                            " arguments in " ^ string_of_expr call))
        else
          let check_call (_, t') en =
            let (et, e') = ex en in
            let err = "Illegal argument found " ^ string_of_type et ^
                " expected " ^ string_of_type t' ^ " in " ^ string_of_expr en in
            ((if et = t' then t' else raise (Failure err)), e')
          in
          let args' = List.map2 check_call fd.params es in
          (fd.ty, SCall(s, args'))
    | Opt(e) ->
        let (t, e') = ex e in
        (Optional t, SOpt(t, e'))
    | EnumCase(s1, s2) ->
        let e = try StringMap.find s1 enums
                with Not_found -> raise (Failure ("Enum " ^ s1 ^ " not defined")) in
        if List.mem s2 e.cases
        then
        (UserDef(e.ename), SEnumCase(s1, s2))
        else raise (Failure ("Enum " ^ s1 ^ " has no case " ^ s2))
    | MethodCall(s, s', es) ->
        let t = try type_of_identifier s scope
                with Not_found -> raise (Failure ("unbound identifier " ^ s)) in
        ...
        let m = try List.find (fun m -> m.fname = s') c.methods
                with Not_found -> raise (Failure ("class " ^ c.cname ^ " has no method " ^ s')) in
        let es' = List.map ex es in
        let offset = List.length c.fields + (index_of m c.methods) in
        (m.ty, SMethodCall(s, s', offset, es'))
    | Self ->
        let (ty, _) = try type_of_identifier "self" scope
                      with Not_found -> raise (Failure("unexpected self at toplevel")) in
        let _ = match ty with UserDef _ -> () | _ -> raise (Failure ("bad type of self")) in
        (ty, SSelf)
    | SelfCall (s, es) ->
        let ty = try type_of_identifier "self" scope
                 with Not_found -> raise (Failure("unexpected self at toplevel")) in
    ...
    | SelfField s ->
        ...
        let cname = match ty with (UserDef s, _) -> s | _ -> raise (Failure ("bad type of self")) in
        let c = StringMap.find cname classes in
        let (_, t, _) = try List.find (fun (s', _, _) -> s = s') c.fields
                        with Not_found -> raise (Failure ("class " ^ c.cname ^ " has no field " ^ s)) in
    ...
    | SelfAssign (s, e) ->
        ...
        let offset = index_of s (List.map (fun (a, _, _) -> a) c.fields) in
        if t = t'
        then (t, SSelfAssign(s, offset, (t', e')))
        else raise (Failure ("bad field assign, expected " ^ string_of_type t ^ " got " ^ string_of_type t'))
    | NoExpr -> (Int, SNoExpr)
    in ex e
  in
  ...
  let stmt scope funcs enums classes check_block s =
    ...
    | Var (n, t, e) ->
        (match scope with
          WithParent(s, map) ->
            ...
            if t = t'
            then (SVar(n, t, e'), newScope)
            else raise (Failure ("Types of 'var' " ^ n ^ " declaration don't match"))
    ...
    | Let (n, t, e) ->
        (match scope with
          WithParent(s, map) ->
            ...
            let newScope = WithParent(s, newCurrScope) in
            if t = t'
            then (SLet(n, t, e'), newScope)
            else raise (Failure ("Types of 'let' " ^ n ^ " declaration don't match"))
        | _ -> raise (Failure "Bad scoping"))
    ...
    | If (c, tblock, fblock) ->
        ...
        if t = Bool
        then (SIf((t, e), List.rev tblock', List.rev fblock'), scope)
        else raise (Failure "'if' condition must be a boolean")
    | IfLet (s, t, e, tblock, fblock) ->
        let (te, e') = expr scope funcs enums classes e in
        let t' = match te with Optional t' -> t'
                          | t' -> raise (Failure("Expected optional in if let, instead got "
                                                    ^ string_of_type t')) in
        ...
        let (fblock', _) = check_block (WithParent(scope, StringMap.empty)) funcs enums classes fblock in
        if t' = t
        then (SIfLet(s, t, (te, e'), List.rev tblock', List.rev fblock'), scope)
        else raise (Failure ("Types of optional and id in if let must match, instead got "
                              ^ string_of_type t ^ " and " ^ string_of_type t'))
    | While (c, body) ->
        let (t, e) = expr scope funcs enums classes c in
        let (body', _) = check_block (WithParent(scope, StringMap.empty)) funcs enums classes body in
        if t = Bool
        then (SWhile((t, e), List.rev body'), scope)
        else raise (Failure "'loop' condition must be a boolean")
    | For (s, e, body) ->
        let (t, e') = expr scope funcs enums classes e in
        let t' = match t with Array t' -> t'
                         | _ -> raise (Failure ("Expression in for loop must be an array")) in
    ...
    in
    check_stmt scope funcs enums classes s
  in
  ...
  let check_function_block_with_check check =
    let rec check_function_block scope funcs enums classes block =
      let rec check_stmt_list scope = function
        [Return _ as s] ->
          let (s', newScope) = check scope funcs enums classes check_function_block s in ([s'], newScope)
      ...
      | []              -> ([], scope)
      in
      let (ss, newScope) = check_stmt_list scope block in (List.rev ss, newScope)
    in
    check_function_block
  in
  ...
  let check_func scope funcs enums classes check_stmt f =
    ...
    let newScope =
      WithParent(scope,
        List.fold_left
        (fun m (s, t) -> StringMap.add s (t, false) m) StringMap.empty params') in
    let newFuncs = StringMap.add fname f funcs in
    let (ss, _) =
      (check_function_block_with_check (check_stmt f.ty))
        (WithParent(newScope, StringMap.empty)) newFuncs enums classes f.body in
    ({
      sfname = fname;
      sparams = params';
      sty = f.ty;
  ...
  let check_enum enums classes e =
    let check_it checked name = 
      let dup_err = "duplicate enum case " ^ name in
      match name with
      | n1 -> match checked with
                    ...
                    | _ -> name :: checked
    in
    let _ = List.fold_left check_it [] (List.sort compare e.cases) in
    let _ = if StringMap.mem e.ename enums || StringMap.mem e.ename classes
               then raise (Failure ("Enum " ^ e.ename ^ " already defined"))
               else () in
    ({ sename = e.ename ; scases = e.cases }, StringMap.add e.ename e enums)
  in
  ...
  let check_inits_ps is =
    ...
    in List.fold_left check_init_params []
        (List.sort compare (List.map (fun (a, _) -> List.map (fun (_, b) -> b) a) is))
  in

  let check_init scope funcs enums classes c (binds, i) =
    let newScope =
      WithParent(scope,
        List.fold_left (fun m (s, t) -> StringMap.add s (t, false) m) StringMap.empty binds) in
    let (i', _) =
      (check_function_block_with_check (check_fstmt (UserDef c))) newScope funcs enums classes i in
    (binds, List.rev i')
  in
  ...
  let check_method scope funcs enums classes check_stmt f =
    ...
    let newScope =
      WithParent(scope,
        List.fold_left (fun m (s, t) -> StringMap.add s (t, false) m) StringMap.empty params') in
    let (ss, _) =
      (check_function_block_with_check (check_stmt f.ty))
        (WithParent(newScope, StringMap.empty)) funcs enums classes f.body in
    ({
      sfname = fname;
      sparams = params';
      sty = f.ty;
  ...
  let check_method_names ms =
    ...
    let check_methods checked m =
      match checked with
        [] -> m :: checked
      | m' :: _ -> if m.fname = m'.fname
                   then raise (Failure ("Class contains duplicate methods " ^ m.fname))
                   else m :: checked
  ...
  let check_cls scope funcs enums classes c =
    ...
    let _ =
      if StringMap.mem cname classes
      then raise (Failure ("class " ^ cname ^ " already defined"))
      else () in
    let _ =
      if StringMap.mem cname enums
      then raise (Failure ("enum with name " ^ cname ^ " already defined"))
      else () in
    ...
    let fields'' =
      ...
      (fun (a, b) ((b', se) as e) ->
        if b = b' || se = SNoExpr
        then (a, b, e)
        else raise (Failure ("Bad field assign")))
      binds fields' in
    let _ = check_inits_ps inits in
    let _ = check_method_names methods in
    let methodScope =
      List.fold_left (fun m (n, t, _) -> StringMap.add n (t, false) m) StringMap.empty fields'' in
    ...
    let methods' =
      List.map
        (fun m -> let (m', _) = check_method newScope funcs enums newClasses check_fstmt m in m') methods in
  ...
  let check_defn scope funcs enums classes = function
    Stmt s  ->
      let (s', newScope) = stmt scope funcs enums classes (check_toplevel_block_with_check stmt) s in
        (SStmt(s'), newScope, funcs, enums, classes)
  | Func_defn f ->
      let _ =
        if StringMap.mem f.fname funcs
        then raise (Failure ("Function " ^ f.fname ^ " already defined"))
        else () in
      let (f', newFuncs) = check_func scope funcs enums classes check_fstmt f in
      (SFunc_defn f', scope, newFuncs, enums, classes)
  | Enum_defn e ->
      let (e', newEnums) =  check_enum enums classes e in
        (SEnum_defn(e'), scope, funcs, newEnums, classes)
  | Cls_defn c ->
      let (c', newClasses) = check_cls scope funcs enums classes c in
        (SCls_defn(c'), scope, funcs, enums, newClasses)
  in
  ...
  let check_with_scope (newDefns, scope, funcs, enums, classes) d =
    ...
    (d'::newDefns, newScope, newFuncs, newEnums, newClasses)
  in

  let (defns', _, _, enums, classes) =
    List.fold_left check_with_scope
      ([], WithParent(Root, StringMap.empty), built_in_decls, StringMap.empty, StringMap.empty)
      defns
  in
  let enums = List.map (fun (a, _) -> a) (StringMap.bindings enums) in
  let classes = List.map (fun (_, c) -> c) (StringMap.bindings classes) in
  (List.rev defns', enums, classes)
